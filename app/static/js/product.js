  ...
  $("#category").on("change", function () {
    ...
    $.getJSON(`/brands/brands/${catId}`, function (brands) {
      ...
      $.each(brands, function (i, brand) {
        if (window.IS_ADMIN && brand.created_by) {
          options += `<option value="${brand.id}">${brand.name} ~ ${brand.created_by}</option>`;
        } else {
        ...
        success: function (res) {
          if (res.success) {
            ...
            <div class="form-check form-switch">
              <input class="status-toggle form-check-input" type="checkbox" id="toggle-${
                product.id
              }" data-id="${product.id}" ${
            ...
            <button class="btn btn-danger btn-sm btn-delete">Delete</button>`;
              ...
              // Conditionally build rowData to include product ID only if admin
              let rowData;
              if (isAdmin) {
                rowData = [
                  product.id,
